...
public partial class PlayerController
{
    private void CalculateMovement()
    {
        ...
        else if (_moveVelocity.sqrMagnitude > 0 && !stopRotate && !UseRootMotion)
        {
    ...
    public void EvasionEnd()
    {
        if (_isGuard)
        {
            _parryingCollider.EnableCollider(true);
        }
    ...
    public void EndRootMotion()
    {
        ...
        byte result = Convert.ToByte(!_isGuard && !LockOnUI.Instance.gameObject.activeSelf);
        SetHandLIK(result, .3f);
    }
    ...
    private async void Evasion()
    {
        if (_isGuard)
        {
            _parryingCollider.EnableCollider(false);
        }
    ...
    private void Guard(bool isGuard)
    {
        ...
        if (isGuard)
            PlayRaiseSwordSound();
        _parryingCollider.SetParryingAble(false);
