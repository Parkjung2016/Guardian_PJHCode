...
public partial class PlayerController
{
    ...
    private void HandleLockOnEvent()
    {
        ...
        LockOn(!LockOnUI.Instance.gameObject.activeSelf);
    }
    ...
    private void HandleRunEvent(bool isRunning)
    {
        ...
        if (LockOnUI.Instance)
            if (LockOnUI.Instance.gameObject.activeSelf && !_isGuard && !_isParrying && input.sqrMagnitude > 0 &&
    ...
    private void HandleEvasionEvent()
    {
        if (AnimatorCompo.GetCurrentAnimatorStateInfo(0).IsName("UseSkill") ||
            CurrentEnergy <= _reduceEnergyWhenEvasion || !canEvasion || _isHitting) return;
        Evasion();
    }
